...
<head>
    ...
    <title>Chủ đề môn <?= htmlspecialchars($monHoc['ten_mon']) ?></title>
            ...
            <div class="col-md-9 content">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Chủ đề môn <?= htmlspecialchars($monHoc['ten_mon']) ?></li>
                    </ol>
                </nav>

                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2>Chủ đề môn: <?= htmlspecialchars($monHoc['ten_mon']) ?></h2>
                    <div>
                        <a href="index.php?controller=cauhoi&action=createTopic&mon_hoc_id=<?= $monHoc['id'] ?>" class="btn btn-success me-2">
                            <i class="fas fa-folder-plus"></i> Thêm chủ đề mới
                        </a>
                        <!-- <a href="index.php?controller=cauhoi&action=create&mon_hoc_id=<?= $monHoc['id'] ?>" class="btn btn-success">
                ...
                <?php if (isset($_GET['error'])): ?>
                    <div class="alert alert-danger alert-dismissible fade show">
                        ...
                        $error = $_GET['error'];
                        switch($error) {
                            case 'topic_has_questions':
                                echo 'Không thể xóa chủ đề vì vẫn còn câu hỏi thuộc chủ đề này!';
                                break;
                            default:
                                echo 'Đã xảy ra lỗi!';
                        }
                ...
                <div class="card mb-4">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">
                            <i class="fas fa-folder me-2"></i>Danh sách chủ đề môn <?= htmlspecialchars($monHoc['ten_mon']) ?>
                        </h5>
                    </div>
                    <div class="card-body">
                        <?php if (empty($danhSachChuDe)): ?>
                            <div class="alert alert-info">
                                <i class="fas fa-info-circle me-2"></i>Chưa có chủ đề nào cho môn học này.
                            </div>
                        <?php else: ?>
                            <div class="row">
                                    ...
                                    <div class="col-md-4 mb-4">
                                        <div class="card topic-card h-100">
                                            <div class="card-body text-center">
                                                ...
                                                <h5 class="card-title"><?= htmlspecialchars($chuDe['ten_chu_de']) ?></h5>
                                                <a href="index.php?controller=cauhoi&action=viewByTopic&id=<?= $chuDe['id'] ?>" 
                                                   class="btn btn-primary btn-sm mt-3 w-100 mb-2">
                                                    <i class="fas fa-list me-1"></i> Xem câu hỏi
                                                </a>
                                                <a href="index.php?controller=cauhoi&action=deleteTopic&id=<?= $chuDe['id'] ?>" 
                                                   class="btn btn-danger btn-sm w-100"
                                                   onclick="return confirm('Bạn có chắc muốn xóa chủ đề này?')">
                                                    <i class="fas fa-trash me-1"></i> Xóa chủ đề
                                                </a>
                                            </div>
                                        </div>
                                    </div>
